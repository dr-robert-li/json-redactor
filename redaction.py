...
def redact_pii(text):
    ...
    # Specific address patterns - examples below
    text = re.sub(r'10b,\s*L10\s*333\s*Ann\s*St', '[REDACTED ADDRESS]', text)
    text = re.sub(r'\b504\s+Lavaca\s+St(?:reet)?,\s*Suite\s*1000\b', '[REDACTED ADDRESS]', text)
...
def process_json_file(file_path, export_csv=False):
    try:
        ...
        for comment in data.get('comments', []):
            # Redact PII in different text fields
            if 'body' in comment:
                comment['body'] = redact_pii(comment['body'])
            if 'html_body' in comment:
                comment['html_body'] = redact_pii(comment['html_body'])
            if 'plain_body' in comment:
                comment['plain_body'] = redact_pii(comment['plain_body'])
            
            # Redact IP and location information
            if 'metadata' in comment and 'system' in comment['metadata']:
                system = comment['metadata']['system']
                if 'ip_address' in system:
                    system['ip_address'] = '[REDACTED IP]'
                if 'location' in system:
                    system['location'] = '[REDACTED LOCATION]'
                if 'latitude' in system:
                    system['latitude'] = 0.0
                if 'longitude' in system:
...
def export_to_csv(data, csv_path):
    try:
        ...
        if not comments:
            print(f"No comments found to export to CSV")
            return False
        
        # Define fields to include in CSV
        fields = [
            'id', 'type', 'author_id', 'public', 'created_at',
            'body', 'via.channel'
        ]
        ...
        with open(csv_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            # Write header row
            header = ['comment_id', 'type', 'author_id', 'public', 'created_at', 
                     'body', 'channel']
            ...
            for comment in comments:
                row = [
                    ...
                    comment.get('body', ''),
                    comment.get('via', {}).get('channel', '') if 'via' in comment else ''
                ]
...
def main():
    parser = argparse.ArgumentParser(description='Redact PII from Zendesk JSON files')
    ...
    if args.file:
        ...
        for file_path in args.file:
            ...
            if not file_path.endswith('.json'):
                print(f"Warning: {file_path} does not have a .json extension")
